    ...
    var page = {
        ...
        initGird: function () {
            ...
            for (var i = 0, l = formScheme.data.length; i < l; i++) {
                ...
                for (var j = 0, jl = componts.length; j < jl; j++) {
                    ...
                    if (mainTable == item.table && mainTablePk == item.field) {
                        mainCompontId = 'lr' + item.id.replace(/-/g, '_');
                    }
            ...
            $.each(colFieldMap, function (id, item) {
                if (!!item.compont)
                {
                    var point = { label: item.fieldName, name: 'lr' + item.compontId.toLowerCase().replace(/-/g, '_'), width: parseInt(item.width), align: item.align };
                            ...
                            if (item.compont.dataSource == "0") {
                                point.formatterAsync = function (callback, value, row) {
                                    learun.clientdata.getAsync('dataItem', {
                                        key: value,
                                        itemCode: item.compont.itemCode,
                                ...
                                point.formatterAsync = function (callback, value, row) {
                                    learun.clientdata.getAsync('custmerData', {
                                        url: '/LR_SystemModule/DataSource/GetDataTable?code=' + vlist[0],
                                        key: value,
                                        valueId: vlist[2],
                                        callback: function (_data) {

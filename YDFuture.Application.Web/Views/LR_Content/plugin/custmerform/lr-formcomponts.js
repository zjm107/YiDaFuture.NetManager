...
(function ($, learun) {
    ...
    $.lrFormComponents = {
        ...
        encode: {//单据编码
            init: function () {
                var $html = $('<div class="lr-custmerform-component" data-type="encode" ><i class="fa fa-sticky-note-o"></i>单据编码</div>');
                return $html;
            },
            render: function ($component) {
                $component[0].dfop = $component[0].dfop || {
                ...
                if (compont.dataSource == '0') {
                    $.lrGetDataItem(compont.itemCode, function (data) {
                        $.each(data, function (id, item) {
                            var $point = $('<label><input name="' + compont.id + '" value="' + item.F_ItemValue + '"' + (compont.dfvalue == item.F_ItemValue ? "checked" : "") + ' type="radio">' + item.F_ItemName + '</label>');
        ...
        checkbox: {//多选框
            ...
            renderTable: function (compont, $row) {//使用表单的时候渲染成table
                ...
                if (compont.dataSource == '0') {
                    $.lrGetDataItem(compont.itemCode, function (data) {
                        $.each(data, function (id, item) {
                            var $point = $('<label><input name="' + compont.id + '" value="' + item.F_ItemValue + '"' + (compont.dfvalue == item.F_ItemValue ? "checked" : "") + ' type="checkbox">' + item.F_ItemName + '</label>');
                            $compont.append($point);
                        });
                    });
                }
        ...
        datetimerange: {
            init: function () {
                var $html = $('<div class="lr-custmerform-component" data-type="datetimerange" ><i class="fa fa-calendar"></i>日期区间值</div>');
                return $html;
            },
            render: function ($component) {
                $component[0].dfop = $component[0].dfop || {
                ...
                function register() {
                    if ($('#' + compont.startTime).length > 0 && $('#' + compont.endTime).length > 0) {
                        $('#' + compont.startTime).on('change', function () {
                            ...
                            if (!!st && !!et) {
                                var diff = learun.parseDate(st).DateDiff('d', et);
                                $compont.val(diff);
                            }
                        });
        ...
        upload: {//附件框
            init: function () {
                var $html = $('<div class="lr-custmerform-component" data-type="upload" ><i class="fa fa-paperclip"></i>附件</div>');
                return $html;
            },
            render: function ($component) {
                $component[0].dfop = $component[0].dfop || {
        ...
        girdtable: {
            init: function () {
                var $html = $('<div class="lr-custmerform-component" data-type="girdtable" ><i class="fa fa-table"></i>表格项</div>');
                return $html;
            },
            render: function ($component) {
                $component[0].dfop = $component[0].dfop || {
        ...
        currentInfo: {//固定信息
            init: function () {
                var $html = $('<div class="lr-custmerform-component" data-type="currentInfo" ><i  class="fa fa-book"></i>固定信息</div>');
                return $html;
            },
            render: function ($component) {
                $component[0].dfop = $component[0].dfop || {

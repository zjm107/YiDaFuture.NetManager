...
(function ($, learun) {
    ...
    $.lrGetDataItem = function (code, callback) {
        learun.httpAsyncGet(top.$.rootUrl + '/LR_SystemModule/DataItem/GetDetailList?itemCode=' + code, function (res) {
            if (res.code == learun.httpCode.success) {
                callback(res.data);
            }
            else {
                learun.alert.error('数据字典数据获取失败,请重新获取！');
                learun.httpErrorLog(res.info);
            }
        });
    }
    ...
    $.lrGetDataSource = function (code, callback) {
        learun.httpAsyncGet(top.$.rootUrl + '/LR_SystemModule/DataSource/GetDataTable?code=' + code, function (res) {
            if (res.code == learun.httpCode.success) {
                callback(res.data);
            }
            else {
                learun.alert.error('数据源数据获取失败,请重新获取！');
                learun.httpErrorLog(res.info);
            }
        });
    }

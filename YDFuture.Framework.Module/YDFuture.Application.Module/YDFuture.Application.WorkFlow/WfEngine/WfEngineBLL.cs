    ...
    public class WfEngineBLL : WfEngineIBLL
    {
        ...
        private int CalcConfluenceNode(WfNodeInfo wfNodeInfo, WfNodeInfo preWfNodeInfo, WfParameter parameter, bool isOk)
        {
            ...
            try
            {
                ...
                {
                    // 需要清除下此会签其他发布的任务
                    ClearConfluenceTask(wfNodeInfo.id, parameter.processId);
                }
        ...
        /// <returns></returns>
        private void ClearConfluenceTask(string nodeId, string processId)
        {
            try
            {
        ...
        private bool CalcConditionNode(WfNodeInfo wfNodeInfo, WfParameter parameter)
        {
            ...
            try
            {
                    ...
                    foreach (var condition in wfNodeInfo.conditions)
                    {
                        ...
                        switch (condition.compareType)//比较类型1.等于2.不等于3.大于4.大于等于5.小于6.小于等于7.包含8.不包含9.包含于10.不包含于
                        {
                            ...
                            case 2:
                                ...
                                break;
                            case 3:
                                if (Convert.ToInt32(v1) > Convert.ToInt32(condition.value))
                                {
                                    res = true;
                                }
                                break;
                ...
                {
                    wfNodeInfo.conditionSql.Replace("{processId}", parameter.processId);
                    DataTable dataTable = databaseLinkIBLL.FindTable(wfNodeInfo.dbConditionId, wfNodeInfo.conditionSql);
        ...
        private bool GetNextTaskes(WfNodeInfo currentNode, WfTransportType transportType, List<WfTaskEntity> wfTaskList, WfParameter parameter)
        {
            try
            {
                ...
                foreach (var node in nextNodes)
                {
                    if (node.type == "conditionnode")// 条件节点
                    {
                        ...
                        {
                            GetNextTaskes(node, WfTransportType.Agree, wfTaskList, parameter);
                        }
                        else
                        {
                            GetNextTaskes(node, WfTransportType.Disagree, wfTaskList, parameter);
                        }
                    }
        ...
        public WfResult Create(WfParameter parameter)
        {
            ...
            try
            {
                ...
                GetNextTaskes(startNode, WfTransportType.Agree, wfTaskList, parameter);
                ...
                {
                   INodeMethod iNodeMethod = UnityIocHelper.Instance.GetService<INodeMethod>(startNode.iocName);
                    ...
                    else
                    {
                        ...
                        switch (parameter.verifyType)//1.审核同意2.审核不同意3.加签4.加签-同意5.加签-不同意6.确认阅读7.保存草稿(暂时不做处理)
                        {
                            case "1":
                                ...
                                break;
                            case "2":
                                res = GetNextTaskes(currentNode, WfTransportType.Disagree, wfTaskList, parameter);
                        ...
                        switch (parameter.verifyType)//1.审核同意2.审核不同意3.加签4.加签-同意5.加签-不同意6.确认阅读7.保存草稿(暂时不做处理)
                        {
                            ...
                            case "6":
                                ...
                                {
                                    INodeMethod iNodeMethod = UnityIocHelper.Instance.GetService<INodeMethod>(currentNode.iocName);
                                    iNodeMethod.Sucess(parameter.processId);
                                }
                                break;
                            case "2":

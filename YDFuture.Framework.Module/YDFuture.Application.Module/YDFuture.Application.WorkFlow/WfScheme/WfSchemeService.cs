    ...
    public class WfSchemeService : RepositoryFactory
    {
        ...
        public IEnumerable<WfSchemeInfoEntity> GetSchemeInfoPageList(Pagination pagination, string keyword, string category)
        {
            try
            {
                ...
                strSql.Append(" FROM LR_Workflow_SchemeInfo t LEFT JOIN LR_Workflow_Scheme t1 ON t.F_SchemeId = t1.F_Id WHERE 1=1 AND t.F_DeleteMark = 0 ");
        ...
        public IEnumerable<WfSchemeInfoEntity> GetCustmerSchemeInfoList(UserInfo userInfo)
        {
            try
            {
                ...
                strSql.Append("SELECT ");
                strSql.Append(schemeInfoFieldSql);
                strSql.Append(" FROM LR_Workflow_SchemeInfo t WHERE 1=1 AND t.F_DeleteMark = 0 AND t.F_EnabledMark = 1  AND t.F_Kind = 1 AND t.F_Id in " + schemeinfoIds);
        ...
        public IEnumerable<WfSchemeEntity> GetWfSchemeList(string schemeInfoId)
        {
            try
            {
                ...
                strSql.Append(" FROM LR_Workflow_Scheme t WHERE 1=1 ");
        ...
        public WfSchemeInfoEntity GetWfSchemeInfoEntityByCode(string code)
        {
            try
            {
                return this.BaseRepository().FindEntity<WfSchemeInfoEntity>(t => t.F_Code == code);
            }
            catch (Exception ex)
            {
                if (ex is ExceptionEx)
